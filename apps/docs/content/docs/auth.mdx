...
PolicyFlow uses **magic-link authentication** — no passwords required. This reduces the attack surface (no password database to breach) and eliminates password-reset flows.
...
One-time in spirit — the server validates but doesn't mark tokens as used (acceptable for MVP). For stricter security, store used token hashes in the database.
...
|---|---|
| No password storage | ✅ |
| Short-lived magic links (24h) | ✅ |
| HMAC-signed JWTs | ✅ |
| Role-based access control | ✅ |
| One-time magic links (stored invalidation) | 🔜 Roadmap |
...
<Callout>
  PolicyFlow uses **admin-provisioned accounts** — users cannot self-register. A SuperAdmin or DeptAdmin creates the account; the user receives a welcome email.
</Callout>
...
3. All existing sessions are immediately invalidated — all users will need to log in again via magic link

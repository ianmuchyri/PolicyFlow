...
PolicyFlow has three roles with strictly enforced scopes:

| Role | Scope | Can do |
|---|---|---|
| **SuperAdmin** | Organization-wide | Manage all users, departments, and policies |
| **DeptAdmin** | Own department | Manage dept users; create/edit dept-scoped policies |
| **Staff** | Read-only | View accessible policies; acknowledge published versions |

Department admins cannot create organization-wide policies, escalate another user to SuperAdmin, or manage users outside their department — all enforced server-side.
